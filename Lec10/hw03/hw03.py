...
def pingpong(n):
    ...
    True
    """
    def pingpong_bottom_up(result, i, step):
        if i == n:
            return result
        else:
            if num_eights(i) > 0 or i % 8 == 0:
                return pingpong_bottom_up(result - step, i+1, -step)
            else:
                return pingpong_bottom_up(result + step, i+1, step)

    return pingpong_bottom_up(1, 1, 1)
...
def count_coins(change):
    ...
    True
    """
    def helper(change, max_value):
        if max_value == 1:
            return 1
        elif change >= max_value:
            return helper(change - max_value, max_value) + helper(change, next_smaller_coin(max_value))
        else:
            return helper(change, next_smaller_coin(max_value))
...
def beaver(f):
#     the first category
    (lambda g: g(g(g(g(g(g(g(f))))))))(lambda h: lambda: h() or h() or h())()
#   73 characters and 3**7 calls

def beaver2(f):
#     the second category
    (lambda g: g(g(g(g(g(g(g(g(g(g(g(g(g(f))))))))))))))(lambda h: lambda: h() or h() or h() or h())()
#   99 characters and 4**13 calls
